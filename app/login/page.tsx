 ...
 *  - The OAuth URL is now built dynamically so the `?from=` query param
 *    (set by proxy.ts when redirecting an unauthenticated user) is
 *    forwarded as the OAuth `state` param. discord-callback reads it
 *    back and sends the user to the page they originally requested.
 *  - Added a `forbidden` error message for proxy.ts permission redirects.
 */
...
const errorMessages: Record<string, string> = {
  ...
  forbidden: 'Your role does not have permission to view that page.',
...
function LoginContent() {
  ...
  const oauthUrl = useMemo(() => {
    ...
    // Carry the originally requested page through OAuth so
    // discord-callback can send the user back to it after login.
